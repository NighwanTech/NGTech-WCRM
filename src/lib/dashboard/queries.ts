...
export async function loadMetrics(db: DB): Promise<MetricsBundle> {
  ...
  return {
    ...
    messagesSentToday: {
      current: (messagesToday.count ?? 0) + (botMessagesToday.count ?? 0),
      previous: (messagesYesterday.count ?? 0) + (botMessagesYesterday.count ?? 0),
    },
    messagesSentByAgentToday: {
      current: messagesToday.count ?? 0,
      previous: messagesYesterday.count ?? 0,
    },
    messagesSentByBotToday: {
      current: botMessagesToday.count ?? 0,
      previous: botMessagesYesterday.count ?? 0,
    },
  }
}
...
export async function loadQuoteAnalytics(db: DB): Promise<QuoteAnalytics> {
  const { data, error } = await db.from('quotes').select('status, amount')
  if (error) throw error
  ...
  return {
    ...
    acceptedCount,
    rejectedCount,
    totalValue: pendingValue + acceptedValue + rejectedValue,
