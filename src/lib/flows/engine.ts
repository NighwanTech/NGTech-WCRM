...
async function advanceFromNodeKey(
  ...
  for (let safety = 0; safety < 64; safety += 1) {
    ...
    if (node.node_type === "delay") {
      ...
      const hours = cfg.delay_hours || 1;
      const resumeAt = new Date(Date.now() + hours * 60 * 60 * 1000).toISOString();
      ...
      await db
        ...
        .update({
          status: "paused",
          resume_at: resumeAt,
          current_node_key: cfg.next_node_key || null,
...
// ============================================================
// Cron trigger — resumes delayed runs.
// ============================================================
...
export async function resumePausedRuns(): Promise<number> {
  ...
  // Find runs that are paused and past their resume_at time.
  ...
  if (error || !runs?.length) return 0;

  let resumed = 0;
  for (const row of runs) {
    const run = row as FlowRunRow;
    ...
    if (Array.isArray(activated) && activated.length > 0) {
      // Advance.
      await advanceFromNodeKey(db, run, run.current_node_key, nodes);
      resumed += 1;
    }
  }
