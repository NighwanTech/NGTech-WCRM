...
export async function POST(request: Request) {
  try {
    ...
    // Use service role so the check works even when RLS would
    // block reading the new plan/status columns.
    try {
      ...
      // send rather than silently break existing users.
    }
    ...
    void incrementMessageUsage(accountId)
