...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as
    | {
        ...
        /** If set, duplicate an existing flow for this user. */
  ...
  if (body.clone_flow_id) {
    const [{ data: sourceFlow }, { data: sourceNodes }] = await Promise.all([
      admin.from('flows').select('*').eq('id', body.clone_flow_id).maybeSingle(),
      admin.from('flow_nodes').select('*').eq('flow_id', body.clone_flow_id),
    ])

    if (!sourceFlow) {
      return NextResponse.json({ error: 'Source flow not found' }, { status: 404 })
    }
    if (sourceFlow.account_id !== accountId) {
    ...
    const { data: flow, error: flowErr } = await admin
      ...
      .insert({
        ...
        status: 'draft',
        trigger_type: sourceFlow.trigger_type,
        trigger_config: sourceFlow.trigger_config,
        entry_node_id: sourceFlow.entry_node_id,
        fallback_policy: sourceFlow.fallback_policy,
      })
      .select()
      .single()
    ...
    if (flowErr || !flow) {
      return NextResponse.json(
        { error: flowErr?.message ?? 'flow clone insert failed' },
        { status: 500 },
      )
    }
