...
export interface TimelineEvent {
  id: string
  title: string
  category: 'AI_ACTION' | 'CUSTOMER_ACTIVITY' | 'FINANCE_EVENT' | 'MARKETING_EVENT' | 'SALES_EVENT'
  timestamp: string
  actor: string
}

/**
 * PRD v16.0 — Universal Activity Timeline Component
 * Standardized activity timeline across Lead, Deal, Campaign, Invoice, and Customer.
 */
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <CardTitle className="text-xs font-bold uppercase tracking-wider text-foreground">
              Universal Activity Timeline & History
            </CardTitle>
            <CardDescription className="text-[10px]">
              Unified cross-object event log for AI decisions, customer interactions & payments
            </CardDescription>
          </div>
        </div>
      ...
      <CardContent className="p-4 space-y-3 font-mono">
        {events.length > 0 ? (
          ...
          <div className="py-8 px-4 text-center border rounded-xl bg-muted/10 space-y-2">
            ...
            <p className="font-bold text-foreground text-xs">No Recent Customer Activity</p>
            <p className="text-[11px] text-muted-foreground max-w-sm mx-auto">
              Customer timeline events and AI actions will record here automatically.
            </p>
          </div>
        )}
      </CardContent>
